const STARTER_KEY = "researchStarter_v1";
...
function autosave(){
  ...
  saveTimer=setTimeout(()=>{
    localStorage.setItem(STARTER_KEY,JSON.stringify(collect()));
    $("#autosave-status").textContent="保存しました";
    setTimeout(()=>$("#autosave-status").textContent="自動保存 ON",1200);
  },300);
}
...
$("#starter-form").addEventListener("submit",e=>{
  e.preventDefault();
  const data=collect();
  localStorage.setItem(STARTER_KEY,JSON.stringify(data));
  toast("問いの成長記録を保存しました！");
});
$("#export-starter").addEventListener("click",()=>{
  const blob=new Blob([JSON.stringify(collect(),null,2)],{type:"application/json"});
  const a=document.createElement("a"); a.href=URL.createObjectURL(blob); a.download="research_starter.json"; a.click();
  setTimeout(()=>URL.revokeObjectURL(a.href),1000);
});

try{
  const saved=JSON.parse(localStorage.getItem(STARTER_KEY)||"null");
  hydrate(saved||{});
}catch{hydrate({});}
